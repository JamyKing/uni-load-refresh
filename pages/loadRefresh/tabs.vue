    ...
    <view class="content">
        ...
        <view class="uni-swiper-tab">
            <block v-for="(tab, index) in tabBars" :key="index">
                <view class="swiper-tab-list" :class="{'active':tabIndex === index}" @click="tabtap(index)">
                    {{tab.name}}
                </view>
            </block>
        </view>
        ...
        <swiper :style="getHeight" :current="tabIndex" @change="tabChange" :duration="1000">
            <swiper-item v-for="(items, index) in tabBars" :key="index">
                <load-refresh :ref="'loadRefresh' + index" :isRefresh="true" heightReduce="80" backgroundCover="#F3F5F5"
                    :currentPage="items.currentPage" :totalPages="items.totalPages" @loadMore="loadData(10)"
                    @refresh="loadData(20)">
        ...
        methods: {
            ...
            loadData(code) {
                ...
                setTimeout(() => {
                    ...
                    tabBars[tabIndex].list.splice(code === 10 ? length : 0, code === 10 ? 0 : length, ...
                        newArray)
                    // 更新页码
